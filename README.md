...
### Funcionalidades
- 📋 **Catálogo** con tarjetas filtrables por reglamento y rol
- 🔍 **Búsqueda** por nombre, clase o rol
- 📖 **Visor de documentos** con 6 pestañas por build (guía, conjuros, roadmap, inventario, objetos, bastión)
- 📊 **Radar chart** de estadísticas por build (DPR / EHP / Control / Soporte / Complejidad)
- ⚖️ **Comparador** de hasta N builds en gráfico superpuesto + tabla
- 🎚️ **Slider de nivel** para mostrar progresión
- 🔗 **Enlace a YouTube** cuando el build tiene vídeo de referencia
...
├── builds.json             # Índice de todos los builds
│
└── <nombre-del-build>/     # Una carpeta por build
    ...
    ├── item list to obtain.md  # Objetos mágicos a conseguir (por tier)
    └── bastion and downtime.md # Bastión, tiempo muerto, facciones, logística
```
...
2. Añadir la entrada en `builds.json`.
3. Recargar la app — aparece automáticamente.
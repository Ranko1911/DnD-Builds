# Objetos a Obtener — Fartificer

**Orden de prioridades:** `1` > `2` > `3` > `Alta` > `Media` > `Baja`

### ⚙️ Gestión de Ranuras de Sintonización (Max 3 Activas)

* **Tier 1 (Niveles 1-4):** Ballesta Retornable (Repeating Shot) (1 slot), Cloak of Protection (1 slot). (Total: 2).
* **Tier 2 (Niveles 5-10):** Repeating Shot Crossbow (1 slot), Ring of Protection (1 slot), Cloak of Protection (1 slot). (Total: 3).
* **Tier 3 (Niveles 11-16):** Cloak of Displacement (1 slot), Amulet of Health (1 slot), Ring of Protection (1 slot). (Total: 3).
* **Tier 4 (Niveles 17-20):** Cloak of Displacement (1 slot), Ring of Spell Turning (1 slot), Ring of Protection (1 slot). (Total: 3).

---

## 🟢 Tier 1 (Niveles 1-4) — Poco Comunes

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Cloak of Protection** | Sí | [⚔️ Sí] | **Priorizar 1:** +1 a la CA y salvaciones. |
| **Goggles of Night** | No | [—] | Prioridad Media. Otorga visión en la oscuridad a 60 pies. Obtenible vía infusión. |

---

## 🔵 Tier 2 (Niveles 5-10) — Raros

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Ring of Protection** | Sí | [⚔️ Sí] | **Priorizar 1:** +2 a la CA y salvaciones. Acumula con la capa. |
| **Cloak of Displacement** | Sí | [⚔️ Sí] | **Priorizar 2:** Los ataques enemigos contra ti se realizan con desventaja. Excelente para un francotirador. |
| **Bag of Holding** | No | [—] | Prioridad Media. Obtenible gratis vía infusión. |

---

## 🟣 Tier 3 (Niveles 11-16) — Muy Raros

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Amulet of Health** | Sí | [⚔️ Sí] | **Priorizar 1:** Establece tu Constitución en 19 (+4), elevando tu HP total y asegurando tus salvaciones de concentración. |
| **Heavy Crossbow +3** | No | [—] | Prioridad Alta. Sustituye la infusión si se consiguen flechas/saetas infinitas ordinarias, aunque Repeating Shot sigue siendo superior por comodidad de carga. |

---

## 🟠 Tier 4 (Niveles 17-20) — Legendarios y Artefactos

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Ring of Spell Turning** | Sí | [⚔️ Sí] | **Priorizar 1:** Ventaja en salvaciones de conjuros dirigidos individuales e intenta devolverlos. |
| **Ioun Stone of Greater Absorption** | Sí | [—] | Prioridad Media. Absorbe hasta 50 niveles de conjuros que te tomen como objetivo antes de destruirse. |

---

## 🔵 Objetos Útiles Sin Sintonización (No-Attunement)

| Objeto | Rareza | Prioridad / Notas |
| :--- | :---: | :--- |
| **Plate Armor +3** | Raro | **Prioridad 1:** Eleva tu CA base a 19 sin gastar espacio de sintonización. |
| **Manual of Quickness of Action** | Muy Raro | **Prioridad Alta:** +2 permanente a la Destreza (superando el límite de 20). |

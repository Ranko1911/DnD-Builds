# Item List — The Furious Paradox

**Orden de prioridades:** `1` > `2` > `3` > `Alta` > `Media` > `Baja`

> **Filosofía de objetos:** Dado que usas Unarmored Defense, priorizas objetos que mejoren tu Constitución y tu Destreza (para aumentar tu CA base), que te den bonificaciones directas de CA sin armadura (como Bracers of Defense) y que aumenten tu Inteligencia.

### ⚙️ Gestión de Ranuras de Sintonización (Max 3 Activas)
*   **Tier 1 (Niveles 1-4):** Sin objetos sintonizados.
*   **Tier 2 (Niveles 5-10):** Cloak of Protection (1 slot), Bracers of Defense (1 slot).
*   **Tier 3 (Niveles 11-16):** Amulet of Health (1 slot), Bracers of Defense (1 slot), Ring of Protection (1 slot). *(La Cloak se desplaza por el Amulet)*.
*   **Tier 4 (Niveles 17-20):** Amulet of Health (1 slot), Bracers of Defense (1 slot), Ring of Protection (1 slot), Tome of Quickness of Action (consumible), Tome of Clear Thought (consumible).

---

## 🟢 Tier 1 (Niveles 1-4) — Poco Comunes

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Goggles of Night** | No | — | Prioridad Alta: Visión en la oscuridad a 60 ft. si tu elfo necesita más alcance. |
| **Weapon +1 (Estoque)** | No | — | **Priorizar 1:** +1 a las tiradas de ataque y daño cuerpo a cuerpo. |

---

## 🔵 Tier 2 (Niveles 5-10) — Raros

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Bracers of Defense** | Sí | ⚔️ Sí | **Priorizar 1:** +2 a tu CA mientras no lleves armadura ni uses escudo. Sinergiza perfectamente con Unarmored Defense de Bárbaro y tu Bladesong (+2 CA neto). |
| **Cloak of Protection** | Sí | ⚔️ Sí | **Priorizar 2:** +1 a CA y tiradas de salvación. |
| **Weapon +2 (Estoque)** | No | — | Prioridad Alta: Mejora tu probabilidad de acierto CaC. |

---

## 🟣 Tier 3 (Niveles 11-16) — Muy Raros

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Amulet of Health** | Sí | ⚔️ Sí | **Priorizar 1:** Fija tu CON en 19 (Mod +4). Aumenta tu CA base en +2 (de CON 14 a 19) e incrementa tus HP máximos de forma retroactiva. El objeto individual más fuerte para esta build. |
| **Tome of Clear Thought** | No | Consumido | **Priorizar 2:** Aumento permanente de +2 a Inteligencia. Tu INT sube a 22 (Mod +6), aumentando tu CA durante Bladesong y tu mitigación con Song of Defense. |
| **Tome of Quickness of Action** | No | Consumido | **Priorizar 3:** Aumento permanente de +2 a Destreza. Tu DEX sube a 22 (Mod +6), aumentando tu CA pasiva y tu acierto CaC. |

---

## 🟠 Tier 4 (Niveles 17-20) — Legendarios y Artefactos

| Objeto | Requiere Sintonización | ¿Sintonizado? | Prioridad / Notas |
| :--- | :---: | :---: | :--- |
| **Sun Blade (Estoque)** | Sí | ⚔️ Sí | **Priorizar 1:** Estoque de pura luz solar radiante. +3 a tiradas de ataque y daño, e inflige daño radiante extra contra muertos vivientes. Cuenta como estoque Finesse para Bladesong. |
| **Ring of Protection** | Sí | ⚔️ Sí | **Priorizar 2:** +1 a CA y salvaciones, apilado con tus Bracers of Defense. |
| **Robe of the Archmagi** | Sí | — | Prioridad Baja: No se puede combinar con Unarmored Defense ya que establece tu CA base en 15+DEX (no apila con CON). |

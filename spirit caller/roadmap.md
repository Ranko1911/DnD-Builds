...
```mermaid
graph TD
    A["Enemigo Muerto Viviente"] --> B{"¿Inteligencia < 12?"}
    B -- "Sí (Fácil Control)" --> C["Control Permanente sin más salvaciones"]
    B -- "No (Fuerte Voluntad)" --> D["Realiza salvación cada hora si recibe órdenes"]
    C --> E["Ejemplos: Wight (Int 10), Mummy (Int 6)"]
    D --> F["Ejemplos: Nightwalker (Int 6, pero Carisma alto), Mummy Lord (Int 11)"]
```
